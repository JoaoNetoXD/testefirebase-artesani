...
import { useAuth } from '@/hooks/useAuth'; // Corrected import if it was from AuthContext directly
import type { Order } from '@/lib/types';

export default function AccountOverviewPage() {
  const [recentOrder, setRecentOrder] = useState<Order | null>(null);
  const [loading, setLoading] = useState(true);
  const { currentUser } = useAuth(); // Use currentUser from useAuth

  useEffect(() => {
    const fetchRecentOrder = async () => {
      if (!currentUser?.id) { // Check currentUser from context
        setLoading(false);
        return;
      }
                ...
                {recentOrder.order_items && recentOrder.order_items.length > 0 && (
                  <ul className="space-y-2">
                    {recentOrder.order_items.slice(0, 2).map((item, index) => (
                      <li key={item.id || index} className="flex items-center gap-3 text-sm">
                        <Image 
                          src={item.products?.images?.[0] || 'https://placehold.co/40x40.png'} 
